...
| 레시피 | 감지 조건 | 요지 |
|---|---|---|
| **robust-implementation** | deep-work + deep-review 감지 | Research→Plan→Implement→Test 단계 진행(risk class medium 이상이면 Spec 추가), 문서 승인 게이트 + deep-review-loop APPROVE 판정 + 테스트 통과를 종료조건으로 |
